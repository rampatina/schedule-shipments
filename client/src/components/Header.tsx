...
import { LogoutIcon, UserAddIcon, UserIcon } from "@heroicons/react/outline";
...
function Header() {
  ...
  return (
    <header className="p-5 flex justify-between border-b">
      <div className="">
        <Link to="/">Scheduling Shipments</Link>
      </div>
      <ul className="flex items-center space-x-6 flex-grow justify-center">
        {user ? (
          <li>
            <button
              onClick={signOut}
              className="float-right flex space-x-2 bg-red-600 hover:bg-red-500 active:bg-red-700 px-4 py-2 text-white">
              <LogoutIcon className="h-6" /> <span className="">Logout</span>
            </button>
            <button
              onClick={showProfile}
              className="float-right flex space-x-2 bg-green-600 hover:bg-green-500 active:bg-green-700 px-4 py-2 text-white">
              <UserIcon className="h-6" /> <span className="">Profile</span>
            </button>
          </li>
        ) : (
          <>
